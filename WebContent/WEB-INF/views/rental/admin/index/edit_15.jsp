					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/15" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										<label>用车目的</label>
										<select class="form-control" name="overtimeusecar">
											<option value="">--请选择--</option>
											<c:forEach items="${DRIVER_OVERTIMEUSERCAR }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverOvertimeusecar }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="form-group" role="form">
										...
										<div class="input-group date">
											...
											<input type="text" name="date" id="edate" placeholder="请输入司机加班日期" class="form-control input-date" value="<fmt:formatDate value="${model.driverDate }" pattern="yyyy-MM-dd " />" />
										</div>
									...
									<div class="form-group" role="form">
										<label>加班总金额</label>
										<fmt:formatNumber value="${model.driverOvertimermbtotal}" var="result" pattern="0.00" groupingUsed="false"/>
										<input type="number" name="overtimermbtotal" placeholder="请输入加班总金额" class="form-control" value="${result}" />
									</div>
