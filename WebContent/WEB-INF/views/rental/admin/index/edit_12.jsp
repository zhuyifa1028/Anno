					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/12" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										<label>用户名称</label>
										<input type="input" name="name" placeholder="请输入用户名称" class="form-control" value="${model.driverName }"  />
									</div>								
									
									<div class="form-group" role="form">
										<label>手机号码</label>
										<input type="input" name="tel" placeholder="请输入手机号码" class="form-control" value="${model.driverTel }"  />
									</div>
									<div class="form-group" role="form">
										<label>出行公里数</label>
										<fmt:formatNumber value="${model.driverStartmile }" var="result" pattern="0" groupingUsed="false"/>
										<input type="number" name="startmile" placeholder="请输入出行公里数" class="form-control" value="${result}" />
									</div>
									...
									<div class="form-group" role="form">
										...
										<div class="input-group date">
											...
											<input type="text" name="date" id="edate" placeholder="请输入例保日期" class="form-control input-date" value="<fmt:formatDate value="${model.driverDate }" pattern="yyyy-MM-dd" />" />
										</div>
									...
									<div class="form-group" role="form">
										<label>车牌号码</label>
										<input type="text" name="carnum" placeholder="请输入车牌号码" class="form-control" value="${model.driverCarnum }" />
									</div>
									
									<div class="form-group" role="form">
										<label>是否打卡</label>
										<select class="form-control" name="clock">
										    <option value="">--请选择--</option>
											<c:forEach items="${DRIVER_CLOCK }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverClock }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>							
