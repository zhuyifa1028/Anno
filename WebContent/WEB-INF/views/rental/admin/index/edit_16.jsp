					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/16" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										<label>费用承担</label>
										<select class="form-control" name="rmbassume">
											<option value="">--请选择--</option>
											<c:forEach items="${DRIVER_RMBASSUME }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverRmbassume }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="form-group" role="form">
										<label>费用类型</label>
										<select class="form-control" name="rmbgoal">
											<option value="">--请选择--</option>
											<c:forEach items="${DRIVER_RMBTYPE }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverRmbtype }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="form-group" role="form">
										<label>费用目的</label>
										<select class="form-control" name="rmbtype">
											<option value="">--请选择--</option>
											<c:forEach items="${DRIVER_RMBGOAL }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverRmbgoal }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="form-group" role="form">
										<label>费用</label>
										<fmt:formatNumber value="${model.driverRmb}" var="result" pattern="0.00" groupingUsed="false"/>
										<input type="number" name="rmb" placeholder="请输入费用" class="form-control" value="${result }" />
									</div>
									...
									<div class="form-group" role="form">
										...
										<div class="input-group date">
											...
											<input type="text" name="date" id="edate" placeholder="请输入费用发生日期" class="form-control input-date" value="<fmt:formatDate value="${model.driverDate }" pattern="yyyy-MM-dd" />" />
										</div>
