					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/14" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										...
										<input type="number" name="milenow" placeholder="请输入当前码表公里数" class="form-control" value="${result}"/>
									</div>								
									...
									<div class="form-group" role="form">
										...
										<div class="input-group date">
											...
											<input type="text" name="date" id="edate" placeholder="请输入加油日期及时间" class="form-control input-date" value="<fmt:formatDate value="${model.driverDate }" pattern="yyyy-MM-dd HH:mm:ss" />" />
										</div>
									...
									<div class="form-group">
										<label class="control-label" for="driverClock">支付类型</label>
											<select name="gasrpaytype" id="clock" class="form-control" data-search="true">
													<option value="">--请选择--</option>
													<c:forEach items="${DRIVER_GASRMBTYPE}" var="m">
														<option value="${m.key }" <c:if test="${model.driverGasrpaytype eq m.key }">selected="selected"</c:if>>${m.value }</option>
													</c:forEach>
											</select>
									</div>								
