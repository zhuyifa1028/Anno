					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/1100" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										<label>轮播文件</label>
										<div class="fileinput input-group <c:if test="${empty model.programaAvatar }">fileinput-new</c:if> <c:if test="${not empty model.programaAvatar }">fileinput-exists</c:if>" data-provides="fileinput">
											<div class="form-control" data-trigger="fileinput">
												<i class="glyphicon glyphicon-file fileinput-exists"></i>
												<c:if test="${not empty model.programaAvatar }">
													<c:set value="${fn:split(model.programaAvatar, '/') }" var="avatar" />
													<span class="fileinput-filename">${avatar[fn:length(avatar) - 1] }</span>
												</c:if>
												<c:if test="${empty model.programaAvatar }">
													<span class="fileinput-filename">1920x900</span>
												</c:if>
											</div>
