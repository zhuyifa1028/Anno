					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/17" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										...
										<select class="form-control" name="reason">
											...
											<c:forEach items="${DRIVER_REASON }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.driverReason }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="form-group" role="form">
										<label>旧车公里数</label>
										<fmt:formatNumber value="${model.driverJcarmile}" var="result" pattern="0" groupingUsed="false"/>
										<input type="number" name="jcarmile" placeholder="请输入旧车公里数" class="form-control" value="${result}" />
									</div>
									<div class="form-group" role="form">
										<label>新车车牌号</label>
										<input type="input" name="newcarnum" placeholder="请输入新车车牌号" class="form-control" value="${model.driverNewcarnum }" />
									</div>
									<div class="form-group" role="form">
										<label>新车公里数</label>
										<fmt:formatNumber value="${model.driverNewcarmile}" var="result" pattern="0" groupingUsed="false"/>
										<input type="number" name="newcarmile" placeholder="请输入新车公里数" class="form-control" value="${result}" />
									</div>
									<div class="form-group" role="form">
										<label>换车日期及时间</label>
										<div class="input-group date">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
											<input type="text" name="date" id="edate" placeholder="请输入换车日期及时间" class="form-control input-date" value="<fmt:formatDate value="${model.driverDate }" pattern="yyyy-MM-dd HH:mm:ss" />" />
										</div>
