					...
					<div class="col-lg-12 animated fadeInRight">
						<div class="ibox float-e-margins">
							...
							<div class="ibox-content">
								<form class="form" action="${admin }/rental/edit/25" method="post" novalidate="novalidate">
									...
									<div class="form-group" role="form">
										<label>帮助标题</label>
										<input type="input" name="cartitle" placeholder="请输入帮助标题" class="form-control" value="${model.recordCartitle }"/>
									</div>
									...
									<div class="form-group" role="form">
										...
										<select class="form-control" name="language">
											...
											<c:forEach items="${RECORD_LANGUAGE}" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.recordLanguage }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									<div class="form-group" role="form">
										...
										<select class="form-control" name="isDisplay">
											...
											<c:forEach items="${RECORD_IS_DISPLAY }" var="m">
												<option value="${m.key }" <c:if test="${m.key eq model.recordIsDisplay }">selected="selected"</c:if>>${m.value }</option>
											</c:forEach>
										</select>
									</div>
									<div class="form-group" role="form">
										<label>帮助详情</label>
										<textarea name="carshow" class="summernote">
											${model.recordCarshow }
				                        </textarea>
									</div>
