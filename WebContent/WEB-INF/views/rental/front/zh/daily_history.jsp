...
<body>
 <form class="m-t-150 mui-form-group" method="post" novalidate="novalidate">
	<div class="daily-history-fixed h-80 bg-ffffff">
		...
		<button class="mui-btn f-s-16 mui-btn-link mui-pull-right h-80" type="submit">确认</button>
	</div>
	<header class="mui-bar mui-bar-nav p-rl-30 h-74 bg-2cad6e">
		<h1 class="mui-title f-s-16 l-h-d-74 ft-ffffff">日常记录</h1>
		<a href="../en/daily_history.html" class="mui-btn f-s-14 mui-btn-link mui-pull-right mui-en-zh">En</a>
	</header>
			...
			<div class="mui-slider-group">
				<div id="not" class="mui-slider-item mui-control-content mui-active">
					<div class="table bg-ffffff f-s-14 ft-999999 t-a-c">
						...
						<div class="table-cell h-80 v-a-m">开始时间</div>
						<div class="table-cell h-80 v-a-m">结束时间</div>
						<div class="table-cell h-80 v-a-m">公里数</div>
					</div>
		...
		$(function() {
			...
			$('.mui-form-group').on('tap', '.mui-btn', function() {
	            setTimeout(function() {
					...
					$.post('../zh/daily_history', {chk_value: chk_value,uaffirm:_uaffirm}, function(data) {
						if(data.succeed == true) {
							window.location.href = '../zh/daily_history.html';
						} else {
