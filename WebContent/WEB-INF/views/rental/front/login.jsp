...
<script type="text/javascript">
	...
	<header class="mui-bar mui-bar-nav p-rl-30 h-74 bg-2cad6e">
		<h1 class="mui-title f-s-16 l-h-d-74 ft-ffffff">司机登录</h1>
	</header>
		...
		<form class="m-t-150 mui-form-group"  method="post">
			<div class="mui-input-row h-80 p-rl-40">
				...
				<div class="mui-input-text ">
					<input name="drivertel" type="text" class="f-s-16 " value="" placeholder="请输入您的手机号码" />
				</div>
			</div>
			<div class="mui-input-row h-80 p-rl-40">
				...
				<div class="mui-input-text">
					<input name="driverpwd" id="pwd" type="password" class="f-s-16" placeholder="请输入您的登录密码" />
				</div>
			</div>
			
			<div class="p-rl-40">
				<button class="mui-btn mui-btn-block m-t-80 ft-ffffff f-s-16 bg-2cad6e">登&nbsp;&nbsp;录</button>
		   ...
		   $("#show").click(function(){				
			...
			$('.mui-form-group').on('tap', '.mui-btn', function() {			
	            setTimeout(function() {
	            	...
	            	var _name =$('input[name="drivertel"]').val();
					var _pwd =$('input[name="driverpwd"]').val();
					
					$.post('login', {name: _name, pwd: _pwd}, function(data) {						
						if(data.succeed == true) {
							localStorage.setItem('keyName',_name);
							localStorage.setItem('keyPass',_pwd);
							var reqUrl = "${requestUrl}";
							var openid = data.result.driverOpenid;
							if(openid != null && openid != ''){//如果openid为空，则先去获取openid，再做页面跳转
								...
								var wxapiurl = "wxapi.html?redirectUrl="+reqUrl;
