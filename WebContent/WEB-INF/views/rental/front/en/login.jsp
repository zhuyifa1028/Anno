...
<script type="text/javascript">
	$(document).ready(function(){
	    var strName = localStorage.getItem('en_tel');
	    var strPass = localStorage.getItem('en_pwd');
	    if(strName){
	...
	<header class="mui-bar mui-bar-nav p-rl-30 h-74 bg-2cad6e">
		<h1 class="mui-title f-s-16 l-h-d-74 ft-ffffff">Login</h1>
		<a href="../zh/login.html" class="mui-btn f-s-14 mui-btn-link mui-pull-right mui-en-zh">中</a>
	</header>
		...
		<form class="m-t-150 mui-form-group" method="post" novalidate="novalidate">
			<div class="mui-input-row h-80 p-rl-40">
				...
				<div class="mui-input-text">
					<input name="usertel" type="text" class="f-s-16"  placeholder="Please enter your mobile number" />
				</div>
			</div>
			<div class="mui-input-row h-80 p-rl-40">
				...
				<div class="mui-input-text">
					<input name="userpwd" type="password"  id="pwd" class="f-s-16" placeholder="Please enter your password" />
				</div>
			</div>
			
			<div class="p-rl-40">
				<button class="mui-btn mui-btn-block m-t-80 ft-ffffff f-s-16 bg-2cad6e">Login</button>
				<a class="ft-aaaaaa mui-pull-right m-t-15 f-s-14" href="forget.html">Forget password?</a>
			</div>
		</form>
	</div>
		   ...
		   $("#show").click(function(){				
			...
			$('.mui-form-group').on('tap', '.mui-btn', function() {
				setTimeout(function() {
					...
					$.post('../en/userlogin', {tel: _tel, pwd: _pwd}, function(data) {
						if(data.succeed == true) {
							localStorage.setItem('en_tel',_tel);
							localStorage.setItem('en_pwd',_pwd);
							...
							if(openid != null && openid != ''){//如果openid为空，则先去获取openid，再做页面跳转
								...
								var wxapiurl = "/en/wxapi.html?redirectUrl="+reqUrl;
