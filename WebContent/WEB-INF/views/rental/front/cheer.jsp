...
<body>
	<header class="mui-bar mui-bar-nav p-rl-30 h-74 bg-2cad6e">
		<h1 class="mui-title f-s-16 l-h-d-74 ft-ffffff">加油记录</h1>
		<a href="cheer_history.html" class="mui-btn f-s-14 mui-btn-link mui-pull-right">历史记录</a>
	</header>
			...
			<form class="mui-form-group" method="post">
				...
				<div class="m-t-30 mui-select mui-select-boult mui-select-times" >
						<span class="ft-999999 f-s-16" id="time">
						   <c:if test="${ empty model.driverDate }" >请选择加油时间</c:if>
						   <fmt:formatDate value="${model.driverDate }" pattern="HH:mm" />	
						</span>
				...
				<div class="m-t-30 mui-input">				   
					<input  class="f-s-16" type="number" name="milenow" 
					value="<fmt:formatNumber type="number" pattern="0" value="${model.driverMilenow}" />"				
					 placeholder="请输入加油时路码" />
				</div>
								
				<div class="m-t-30 mui-input ">
					<span class="ft-333333 f-s-16"  id="type" >
					  油卡
					</span>
					<input type="hidden" name="gasrpaytype" value="${ model.driverGasrpaytype}"/>
				</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			// 选择加油方式
			$('.mui-form-group').on('tap', '.mui-select-cheer', function() {
				var _this = $(this);
				var _input = _this.find('input[type="hidden"]');
				var _span = _this.find('span');

				cheer.show(function(rs) {
			...
			$('.mui-form-group').on('tap', '.mui-btn', function() {
	            setTimeout(function() {	
	            	...
	            	if(document.getElementById("time").innerText=="选择时间"){
	            	...
	            	date=date+time;	
	            	var gasrpaytype;
	            	if( document.getElementById("type").innerText=="现金"){
	            		 gasrpaytype='0';
	            	}else if(document.getElementById("type").innerText=="油卡"){
	            		 gasrpaytype='1';
	            	}	          

	            	var milenow=$('input[name="milenow"]').val();	              
	                if(time==""||time==null||gasrpaytype==""||gasrpaytype==null||milenow==""||milenow==null){
	                	mui.toast("数据不完整，请检查！",{ duration:'long', type:'div' });
	                	return  false;
	                }	              
	                $.post('cheer', {_id:_id,date: date,gasrpaytype:gasrpaytype,milenow:milenow}, function(data) {						
						if(data.succeed == true) {	
							window.location.href = 'cheer_history.html';					
						}  else {
