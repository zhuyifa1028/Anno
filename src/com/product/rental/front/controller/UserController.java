...
public class UserController extends BaseController {
				...
				if (StringUtils.isNotBlank(userpwd)) {
					...
					if (users != null && !users.isEmpty() && users.size() == 1) {
						...
						if (user.getUserState() != null && user.getUserState().equals(User.USER_STATE_YES)) {
							json.setMessage("User cannot login , please contact the back office");
						} else {
						...
						json.setMessage("Wrong user name or password");
					}
				} else {
			...
			for (String key : param.keySet()) {
				...
				for (String item : ary) {
					...
					if (Integer.parseInt(item) == 1) {
						json.setMessage("No item has been selected！");
					} else {
	...
	public ModelAndView work_detail(ModelAndView view, HttpServletRequest request, HttpSession session) {
		...
		try {
			...
			if (work != null) {
				...
				map.put("driverIdcard", u.getUserTel());
				...
				for (Driver driver : lists) {
					...
					Driver d = driver;
					if (driver.getDriverUaffirm().equals("0")) {
						if (key.equals(work)) {
			...
			if (StrUtils.isNotEmpty(code)) {
				...
				if (user != null) {
					...
					if (StrUtils.isNotEmpty(redirectUrl))
						...
						view.setViewName("redirect:/en/index.html");
				} else {
					...
					if (uList != null && uList.size() == 1) {
						...
						view.setViewName("redirect:/en/login.html");
					}
	...
	public void wxapi(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			String Url = request.getScheme() + "://" + request.getServerName() + "/en/openid.html?ref=" + ref;
			if (StrUtils.isNotEmpty(redirectUrl))
